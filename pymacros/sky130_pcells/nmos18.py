...
class NMOS18(pya.PCellDeclarationHelper):
    ...
    def draw_guard_ring(self,x,y,guard_width,guard_height,tap_width = 0.26):
    ...
    def produce_impl(self):
        ...
        num_hori_licon_diff,free_space_licon_diff_hor = self.number_spc_contacts(diffusion_width_big,liconpoly_spc,licon_size,licon_size)
        ...
        num_hori_mcon_diff,free_space_mcon_diff_hor = self.number_spc_contacts(diffusion_width_big,0,mcon_spc,mcon_size)
        sh2 = 0
        ...
        nmos_cell.shapes(l_nsdm).insert(nsdm_box)
        ...
        bottom_connection = pya.CellInstArray(down_gate_connection.cell_index(), pya.Trans(pya.Point(0, 0)),
                                              pya.Vector(down_iteration_distance, 0), pya.Vector(iteration_distance_group, 0), down_iteration,
        ...
        upper_connection = pya.CellInstArray(up_gate_connection.cell_index(), pya.Trans(pya.Point(shift_distance, 0)),
                                             pya.Vector(up_iteration_distance, 0), pya.Vector(iteration_distance_group, 0), up_iteration, nf/self.n - math.ceil(nf/(2*self.n)))
        ...
        if self.gr:
            ...
            psdm_guard_ring_path = pya.Path(
                [guard_ring_lower_left, guard_ring_upper_left, guard_ring_upper_right, guard_ring_lower_right,
                 guard_ring_lower_left], tap_width + 2 * npsdm_enc_diff, (tap_width + 2 * npsdm_enc_diff) / 2, 0)
            gurad_ring_metal1_upper = pya.Path ([guard_ring_upper_left,guard_ring_upper_right],tap_width,tap_width/2,tap_width/2)
            self.cell.shapes(l_met1).insert(gurad_ring_metal1_upper)
            gurad_ring_metal1_lower = pya.Path ([guard_ring_lower_left,guard_ring_lower_right],tap_width,tap_width/2,tap_width/2)
            self.cell.shapes(l_met1).insert(gurad_ring_metal1_lower)
            gurad_ring_metal1_right = pya.Path ([guard_ring_lower_right,guard_ring_upper_right],tap_width,tap_width/2,tap_width/2)
            self.cell.shapes(l_met1).insert(gurad_ring_metal1_right)
            gurad_ring_metal1_left= pya.Path([guard_ring_lower_left,guard_ring_upper_left],tap_width,tap_width/2,tap_width/2)
            self.cell.shapes(l_met1).insert(gurad_ring_metal1_left)
            ...
            distance_cont_guard_vert = guard_ring_upper_left.y - guard_ring_lower_left.y
            num_licon_guard_vert, free_spc_licon_guard_left = self.number_spc_contacts(distance_cont_guard_vert, mcon_m1_enc,
                                                                                  licon_size, licon_size)
            ...
            for mcon in range(num_mcon_guard_vert):
                mcon_guard_box_left = pya.Box(mcon_guard_p1_x_left, mcon_guard_p1_y, mcon_guard_p2_x_left, mcon_guard_p2_y)
                mcon_guard_box_right = pya.Box(mcon_guard_p1_x_right, mcon_guard_p1_y, mcon_guard_p2_x_right, mcon_guard_p2_y)
                self.cell.shapes(l_mcon).insert(mcon_guard_box_left)
                self.cell.shapes(l_mcon).insert(mcon_guard_box_right)
                mcon_guard_p1_y +=mcon_size + mcon_spc
                mcon_guard_p2_y +=mcon_size + mcon_spc
            ...
            # mcon_guard_p1_x = guard_ring_lower_left.x - licon_size / 2
            # mcon_guard_p1_y = guard_ring_lower_left.y + free_spc_mcon_guard_left
            # mcon_guard_p2_x = mcon_guard_p1_x + mcon_size
            # mcon_guard_p2_y = mcon_guard_p1_y + mcon_size
            # for mcon in range(num_mcon_guard_left):
            #     mcon_guard_box = pya.Box(mcon_guard_p1_x, mcon_guard_p1_y, mcon_guard_p2_x, mcon_guard_p2_y)
            #     # top_nmos_cell.shapes(l_mcon).insert(mcon_guard_box)
            #     mcon_guard_p1_y += mcon_size + mcon_spc
            #     mcon_guard_p2_y += mcon_size + mcon_spc
            # top_nmos_cell.shapes(l_li).insert(li_box_down)
            # top_nmos_cell.shapes(l_met1).insert(li_box_down)
